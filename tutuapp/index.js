...
function genMonthSlots(ym){ 
  const [y,m] = ym.split("-").map(Number);
  const start = new Date(Date.UTC(y,m-1,1,16,0,0));
  const end = new Date(Date.UTC(y,m,0,20,0,0));
  const days = [1,2,3]; // Lunes=1, Martes=2, Miércoles=3
  const out = [];
...
app.post("/book", async (req,res)=>{
  try{
    ...
    const clash=await db.collection("reservas").where("startISO","==",startISO).limit(1).get();
    if(!clash.empty) return res.status(409).json({error:"slot taken"});
    const endISO=new Date(new Date(startISO).getTime()+30*60000).toISOString();
    const signupToken=uuid();
    const doc={
      nombre:name,email,telefono:phone,
      startISO,endISO,status:"confirmada",
      signupToken,
      signupExpires: admin.firestore.Timestamp.fromDate(new Date(new Date(startISO).getTime()+90*60000)),
      emailMatriculaEnviado:false,
